...
###########################################################################
## GeoIP domain                                                          ##
## Uses a GEO location API to get the country code of an IP             ##
## To disable the GEO location check, set the value to 0                 ##
###########################################################################

geoipdomain="http://ip-api.com/json"
...
function access_log_summary() {
    ...
    if [[ $log ]] ; then
        ...
        sleep 1
        cat $log 2>/dev/null | grep GET | awk -F '"' '{print $2}' | awk '{print $1 " " $2}' | cut -d? -f1 | sort | uniq -c | sort -n | tail -20 | column -t
        echo ""

        echo $(ColorGreen "Top 20 POST requests:")
        sleep 1
        cat $log 2>/dev/null | grep POST | awk -F '"' '{print $2}' | awk '{print $1 " " $2}' | cut -d? -f1 | sort | uniq -c | sort -n | tail -20 | column -t
        echo ""
        ...
        if [[ $geo == 'yes' ]] ; then
            if ! command -v jq &>/dev/null; then
                echo $(ColorRed "Error: jq is required for geo location lookup. Please install jq and try again.")
                exit 1
            fi
            oIFS="$IFS"
            IFS=$'\n'
            for ips in $(cat $log 2>/dev/null | awk '{print $1}' | sort | uniq -c | sort -rn | head -20 | column -t); do
                IFS=' '
                array=($ips)
                hits="${array[0]}"
                ip="${array[1]}"
                if [[ $ip =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
                    location=$(curl -s "$geoipdomain/$ip" | jq -r '.country')
                    if [[ $location == "null" ]]; then
                        location="Unknown"
                    fi
                    echo "$hits - $ip - $location" | column -t
                else
                    echo "$hits - $ip - Invalid IP" | column -t
                fi
            done
            IFS="$oIFS"
        else
            cat $log 2>/dev/null | awk '{print $1}' | sort | uniq -c | sort -rn | head -20 | column -t
        fi
        echo ""
        ...
        if [[ $geo == "yes" ]] ; then
            oIFS="$IFS"
            IFS=$'\n'
            for ips in $(tail -n 1000 $log 2>/dev/null | awk '{print $1}' | sort | uniq -c | sort -rn | head -20 | column -t); do
                IFS=' '
                array=($ips)
                hits="${array[0]}"
                ip="${array[1]}"
            ...
            tail -n 1000 $log 2>/dev/null | awk '{print $1}' | sort | uniq -c | sort -rn | head -20 | column -t
        fi
    else
        echo ""
